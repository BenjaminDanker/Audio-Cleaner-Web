...
class LocalProcessor:
    ...
    def process_video_file(self, job_id, file_path, atten_db=30):
        ...
        try:
            ...
            processor = VideoProcessor(mock_upload, atten_db)
            ...
            try:
                ...
                logger.error(f"Video processing failed for job {job_id}: {processing_error}")
                self.update_job_status(
                    job_id, 
                    'failed', 
                    0, 
                    f'Processing failed: {str(processing_error)}'
                )
