import os, json, asyncio, tempfile, logging, traceback, signal, contextlib, sys
...
# Mode guard: default to 'batch' for this entrypoint
_mode = os.getenv('PROCESSOR_MODE', 'batch').lower()
if _mode not in ('batch', 'both'):
    logging.warning("processor_main started with PROCESSOR_MODE=%s; exiting because this entrypoint is for batch processing.", _mode)
    raise SystemExit(0)
